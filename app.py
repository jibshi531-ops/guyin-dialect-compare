...
st.caption("选择某个语音特征后，地图只高亮出现该特征的县区。选择“区域倾向”时，右侧显示对应统计图。")
...
required_count_cols = ["特征字段", "特征值", "特征数量", key_col]
...
with right_col:
    ...
    if selected_field in ["區域傾向", "区域倾向"]:
        ...
        region_df["特征值"] = region_df["特征值"].fillna("").astype(str).str.strip()
        region_df = region_df[
            (region_df["特征值"] != "") &
        ...
        exclude_unknown = st.checkbox("排除“不明”", value=False)
        ...
        st.info("当前右侧统计图仅对“区域倾向”字段显示。\n\n请在左侧把“选择特征字段”切换为“区域倾向 / 區域傾向”。")
