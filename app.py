...
st.caption("选择某个语音特征后，地图只高亮出现该特征的县区。已去掉在线底图，仅保留广东县区边界，以提高加载速度。")
...
if key_col not in counts.columns:
    st.error("feature_counts.csv 中没有“市县匹配键”字段。")
    st.stop()

if "特征字段" not in counts.columns:
    st.error("feature_counts.csv 中没有“特征字段”字段。")
    st.stop()

if "特征值" not in counts.columns:
    st.error("feature_counts.csv 中没有“特征值”字段。")
    st.stop()

if "特征数量" not in counts.columns:
    st.error("feature_counts.csv 中没有“特征数量”字段。")
    st.stop()
...
current_counts["特征数量"] = current_counts["特征数量"].fillna(0).astype(int)
...
show_detail = st.checkbox("显示当前特征对应的文献记录明细", value=False)
